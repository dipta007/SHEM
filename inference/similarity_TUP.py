...
from dataloaders.similarity_dataset_TUP import SimilarityDatasetTup
...
class SimilarityTup:
    def __init__(self, config, model, path, hard=True):
        ...
        self.dataset = SimilarityDatasetTup(
            self.config,
            [path],
            self.vocab,
            self.vocab2,
    ...
    def test(self, return_all=False):
        ...
        for it, batch in tqdm(enumerate(self.dataloader), total=len(self.dataloader)):
            ...
            ab_sim = cosine_similarity(rp1["p_0"], rp1["q"]) + cosine_similarity(
                rp1["p_1"], rp1["q"]
            )
            if self.hard:
                cd_sim = cosine_similarity(rp2["p_0"], rp2["q"]) + cosine_similarity(
                    rp2["p_1"], rp2["q"]
                )
