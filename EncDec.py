...
class EncDecBase(nn.Module):
    def __init__(self, emb_size, hidden_size, embeddings=None, cell_type="GRU", layers=1, bidir=True, use_cuda=True, base_layer=True):
        """
        Args:
            emb_size: size of word embeddings, i.e. 400/500/600/700
            hidden_size: hidden layer size, i.e. encoder/decoder hidden layer size
            embeddings: Embedding layer for encoder (in_embedding) and decoder (out_embedding)
            cell_type: GRU or LSTM
            layers: number of layers in GRU/LSTM
            bidir: use bidirectional GRU/LSTM or not
...
class Encoder(EncDecBase):
    def forward(self, input, hidden, seq_lens, use_packed=True):
        ...
        Args:
            ...
            use_packed: use packed version of gru/lstm or not
        """
        out = input
        # if self.base_layer:
        out = self.embeddings(input)
        out = out.view(input.shape[0], input.shape[1], -1) # (batch, seq_len, emb_size)
        if use_packed:
...
class Decoder(EncDecBase):
    def __init__(self, emb_size, hidden_size, vocab_size=None, embeddings=None, cell_type="GRU", layers=1, attn_dim=-1, use_cuda=True, dropout=0.0):
        ...
        Args:
            ...
            cell_type: GRU or LSTM
            layers: number of layers in GRU/LSTM
            attn_dim: (latent_dim, dec_hidden_size)
            use_cuda: use of cuda or not
            dropout: if dropout > 0.0, then use dropout
        """
        if attn_dim is None:
    ...
    def forward(self, input, hidden, memory, template_decode_input):
        # print("**** 9", input.size()) # (2)
        out = self.embeddings(input)
        out = out.view(input.shape[0], -1) # (batch, emb_size)

        # print("**** 8", out.size(), input.size()) # (2, 300), (2),
        if self.drop is not None:
            out = self.drop(out) # [batch, emb_size]

        # concat input feed 
        # print("**** 10", out.size(), self.input_feed if self.input_feed is None else self.input_feed.size(), input.min(), input.max()) # (2, 300) (2, 512)
        # print("**** 6", out)
        # print("**** 5", self.input_feed)
        dec_input = torch.cat([out, self.input_feed], dim=1).unsqueeze(dim=1) # [batch, emb_size + attn_dim]
        # print("**** 7", out.size(), input.size(), dec_input.size()) # (2, 300), (2), (2, 1, 812)
        self.rnn.flatten_parameters()
...
class Attention(nn.Module):
    ...
    def forward(self, input, memory, mem_lens=None, template_decode_input=None):
        ...
        # print("**** 6", Wh.size(), memory_t.size(), scores.size())

        if mem_lens is not None: # mask out the pads
            # print("**** 4", memory.size(), memory_t.size(), mem_lens.size(), scores.data.size())
            mask = sequence_mask(mem_lens)
            if self.use_cuda:
                mask = mask.unsqueeze(1).cuda()  # Make broadcastable
            else:
                mask = mask.unsqueeze(1)
            # print("**** 5", scores.data.size(), mask.size(), mem_lens.max())
            # print(mem_lens)
        ...
        context = torch.bmm(scores, memory).squeeze(dim=1) # [batch, dim], context vectors for each batch # line 2
        # cat = torch.cat([context, input], 1) 
        cat = torch.tanh(context) + torch.tanh(Wh.squeeze())
        attn_output = self.linear_out(cat) # line 3
        if self.is_decoder:
            self.template_input=template_decode_input
            logit = self.logits_out(cat)
            logit += self.template_input
            frame_to_vocab = self.logits_out(torch.tanh(memory))
            ...
            # print("**** 99", attn_output.size(), scores.size(), frame_to_frame.size(), vocab_to_frame.size())
