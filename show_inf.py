...
def find_recall(real, passed, l2):
    def preprocess(frames):
        # print(frames)
        frames = list(filter(lambda x: x != '__NOFRAME__', frames))
    ...
    def recall(y, y_hat):
        ...
        print(y)
        print(y_hat)
        print(set(y).intersection(y_hat))
        print()
        return len(set(y).intersection(y_hat)) / len(y)
    ...
    def precision(y, y_hat):
        ...
        print(set(y).intersection(y_hat))
        print()
        return len(set(y).intersection(y_hat)) / len(y_hat)

    # print(real, passed, l2)
    real = preprocess(real)
    passed = preprocess(passed)
    l2 = preprocess(l2)

    global recall_real, recall_pred, precision_real, precision_pred
    recall_real.append(recall(real, l2))
    recall_pred.append(recall(passed, l2))
    precision_real.append(precision(real, l2))
    precision_pred.append(precision(passed, l2))
...
def run(args, args_dict):
    ...
    iteration_wo_pad = 0
    with open(f'./inferences/infer_exp_num_{args.exp_num}_100.txt', 'w') as fp:
            ...
            with torch.no_grad():
                ...
                for i, model in enumerate(models):
                    ...
                    if topics_dict['ref_frames'].count("<pad>") == 0:
                        ...
                        if i == 1:
                            l2 = topics_dict['infered_frames'][:child_per_layer[i]]
                        ...
                        if i == len(models) - 1:    
                            # print(real, passed, l2)
                            find_recall(real, passed, l2)
                    # else:
                    #     print(topics_dict['ref_frames'])
            # break
            if iteration_wo_pad >= 100:
                fp.write(f"Total Iterations: {len(recall_real)}")
                fp.write(f"Recall based on Real Frames:".ljust(40))
                fp.write(f"{sum(recall_real)/len(recall_real)}\n")
                fp.write(f"Recall based on Pred Frames:".ljust(40))
                fp.write(f"{sum(recall_pred)/len(recall_pred)}\n")

                fp.write(f"Precision based on Real Frames:".ljust(40))
                fp.write(f"{sum(precision_real)/len(precision_real)}\n")
                fp.write(f"Precision based on Pred Frames:".ljust(40))
                fp.write(f"{sum(precision_pred)/len(precision_pred)}\n")
                break

# srun --mem=1000 --time=1:00:00 --gres=gpu:1 --pty bash
# readlink -f ferraro_user/SSDVAE_ext/inferences
