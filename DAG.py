...
class LatentNode(nn.Module):
    ...
    def __init__(self, num_Frames, dim, nodeid="0", embeddings=None, use_attn=True, use_cuda=True, nohier_mode=False, train_mode=True, base_layer=True):
        """
        Args
    ...
    def frames_onehot(self, f_vals_clause):
        ...
        # print("frames_clause_size: ", frames_clause_.size(), f_vals_clause.size())
        if self.use_cuda:
        ...
        # print("one_hot_after_size: ", one_hot.size())
        return one_hot

    def frames_multihot(self, f_vals):
        """
        f_vals: [batch, num_clause]
        (Each time one of the frames is called)
        """
    ...
    def infer_(self, input_memory, input_lens, init_query=None, f_vals=None, template_input=None):
        ...
        if not self.isroot() and not self.nohier:  # if we are a child node AND we are not running in nohier mode
            ...
            l_o = Variable((f_val_curr>0).view((-1,1))).float()
            l_c = Variable((f_val_curr>2).view((-1,1))).float()
            ...
            l_o = Variable((f_val_curr>0).view((-1,1))).float() # W/O __NO_FRAME__ elements
            l_c = Variable((f_val_curr>2).view((-1,1))).float() # W/O __NO_FRAME__, <UNK>, <PAD> elements
        ...
        V_raw = V
        V = (1-l_o)*V + (l_o*torch.norm(V) * fval_one_hot.squeeze(0)) # hard injection
        # V = (1-l_o)*V + (l_o * (torch.norm(V) * fval_one_hot.squeeze(0) + V)) # soft injection
        
        # print(fval_one_hot.size(), fval_one_hot.squeeze(0).size(), V.size(), f_val_curr.size(), f_vals.size())
        # print(self.base_layer, (fval_one_hot == 1.).sum(dim=2))
        
        if False and self.base_layer:
            gumbel_samples = F.gumbel_softmax(logits=V, tau=torch.tensor([self.tau]).cuda())
        else:
        ...
        # print(self.base_layer, V_raw.size(), fval_one_hot.size())
    ...
    def num_of_nodes(self):
        """
        Get the number of nodes rooted on self
        """
        num_of_childs = len(self.children)
        for child in self.children:
            num_of_childs += child.num_of_nodes()
...
def example_tree(num_Frames, all_dim, frame_max, padding_idx=None, use_cuda=True, nohier_mode=False, num_of_childs=5, base_layer=True, frame_embedding=None):
    ...
    frame_max: number of total unique frames (in our case same as num_frames)
    padding_idx: index of <pad>
    use_cuda: cuda mode enabled or not
    ?nohier_mode: Not using any more?nohier_mode
    num_of_childs: Number of childs for latent tree (with root)
    """
    ...
    for i in range(num_of_childs):
        ...
        childs[i] = LatentNode(num_Frames, curr_dim, nodeid=id_str, embeddings=frame_embedding,
                        use_cuda=use_cuda, nohier_mode=nohier_mode, base_layer=base_layer)
    
    for i in range(num_of_childs-2, -1, -1):
        childs[i].add_child_(childs[i+1])
